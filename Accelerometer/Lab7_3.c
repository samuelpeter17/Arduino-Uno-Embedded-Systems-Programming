/* ENGS 28
 * Description:	 	Program to detect accelerometer tilt and print it on seven segment display; turn on appropriate LED to show image orientation
 *
...
int main(void) {
	...
	while(1) {
		...
		//read data from accelerometer for x,y,z axes
		...
		if(times == 125){	//check if one second has passed
			...
			//display output on seven segment display
			SevenSeg_number(abs(tilt),display_buffer);
			if(tilt<0){
				display_buffer[0] = 0b01000000;	//-ve sing if angle is negative
				SevenSeg_write(display_buffer);
			}else{
				SevenSeg_write(display_buffer);
			}
